...
class dadata
{
    ...
    function __construct(modX &$modx, array $config = array())
    {
        ...
        $corePath = $this->modx->getOption('dadata_core_path', $config,
            $this->modx->getOption('core_path') . 'components/dadata/');
        $assetsUrl = $this->modx->getOption('dadata_assets_url', $config,
            $this->modx->getOption('assets_url') . 'components/dadata/');
    ...
    public function setConfig($config = array())
    {
        ...
        $this->mode = trim($this->getOption('apiMode', $this->config,
            $this->modx->getOption('dadata_apiMode', null, 0, true), true));
        $this->token = trim($this->getOption('apiToken', $this->config,
            $this->modx->getOption('dadata_apiToken', null, 0, true), true));
        $this->secret = trim($this->getOption('apiSecret', $this->config,
            $this->modx->getOption('dadata_apiSecret', null, 0, true), true));
        $this->url = trim($this->getOption('apiUrl', $this->config,
            $this->modx->getOption('dadata_apiUrl', null, 0, true), true));

        switch (true) {
            case $this->url:
                break;
            case !$this->url AND !$this->mode:
                $this->url = trim($this->getOption('apiUrlFree', $this->config,
                    $this->modx->getOption('dadata_apiUrlFree', null, 'https://dadata.ru/api/v2', true), true));
                break;
            case !$this->url AND $this->mode:
                $this->url = trim($this->getOption('apiUrlPay', $this->config,
                    $this->modx->getOption('dadata_apiUrlPay', null,
                        'https://suggestions.dadata.ru/suggestions/api/4_1/rs', true), true));
                break;
        }
    }
    ...
    public function loadCustomJsCss($objectName = 'modxDaData', $configName = 'dadata')
    {
        ...
        if (!isset($this->modx->loadedjscripts[$objectName])) {
            ...
            $this->modx->regClientScript("<script type=\"text/javascript\">{$objectName}.initialize({$configName}.opts);</script>",
                true);
     ...
     * @return mixed
     */
    public function processStandardData(array $data = array(), array $request = array())
    {
        if (!$this->Format) {
            $this->loadFormat();
        }

        return $this->Format->processStandardData($data, $request);
    }
     ...
     * @return array
     */
    public function cleanField($field, $type = '', array $opts = array())
    {
        $opts = array($field);
        $request = $this->getRequest($type, $opts);
        $this->showLog($request);

        $cacheResponse = $this->getOption('cacheResponse', $this->config, 0, true);

        if ($cacheResponse) {
            $options = array(
                'cache_key' => 'dadata/query/' . $type . '/' . sha1(serialize($request)),
                'cacheTime' => 0,
            );
            if (!$data = $this->getCache($options)) {
                $data = $this->query('clean/' . $type, $opts);
                if (isset($data[0])) {
                    $data = array('standard' => $data[0]);
                }
                $this->setCache($data, $options);
            }
            if ($this->getOption('isprocess_data', null, true, true)) {
                $data = $this->processStandardData($data, $request);
            }
        } else {
            $data = $this->query('clean/' . $type, $opts);
            if (isset($data[0])) {
        ...
        return isset($data['standard']) ? $data : array();
    }
     ...
     * @return array
     */
    public function cleanAddresses($address, array $opts = array())
    {
        return $this->cleanField($address, 'address', $opts);
    }
     ...
     * @return array
     */
    public function cleanPhone($phone, array $opts = array())
    {
        return $this->cleanField($phone, 'phone', $opts);
    }
     ...
     * @return array
     */
    public function cleanPassport($passport, array $opts = array())
    {
        return $this->cleanField($passport, 'passport', $opts);
    }
     ...
     * @return array
     */
    public function cleanName($name, array $opts = array())
    {
        return $this->cleanField($name, 'name', $opts);
    }
     ...
     * @return array
     */
    public function cleanEmail($email, array $opts = array())
    {
        return $this->cleanField($email, 'email', $opts);
    }
     ...
     * @return array
     */
    public function cleanBirthdate($birthdate, array $opts = array())
    {
        return $this->cleanField($birthdate, 'birthdate', $opts);
    }
     ...
     * @return array
     */
    public function cleanVehicle($vehicle, array $opts = array())
    {
        return $this->cleanField($vehicle, 'vehicle', $opts);
    }
     ...
     * @return array
     */
    public function cleanStructure(array $structure = array(), array $data = array(), array $opts = array())
    {
        $opts = array(
            'structure' => $structure,
            'data'      => $data
    ...
    public function showLog($message = '', $show = false)
    {
        if (($show OR !empty($this->config['showLog']))) {
            $this->modx->log(modX::LOG_LEVEL_ERROR,
                print_r('[' . $this->namespace . '] ' . (($show) ? 'show' : ''), 1));
