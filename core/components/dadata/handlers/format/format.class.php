...
class Format implements FormatInterface
{
    ...
    public function __construct($modx, &$config)
    {
        ...
        $corePath = $this->modx->getOption('dadata_core_path', null,
            $this->modx->getOption('core_path', null, MODX_CORE_PATH) . 'components/dadata/');
        /** @var dadata $dadata */
     ...
     * @return array
     */
    public function processStandardData(array $standard = array(), array $request = array())
    {
        $this->request = $request;

        return $this->processSuggest($standard);
    }
     ...
     * @return array
     */
    public function formatStandard($val = '', array $data = array())
    {
        $return = $this->getOption('return', $this->request, false, true);
        if (!$return) {
            return $val;
     ...
     * @param array  $data
     */
    public function formatDetail($val = '', array $data = array())
    {
        $this->modx->log(modX::LOG_LEVEL_ERROR, "[dadata] Error: " . $val);
    }
